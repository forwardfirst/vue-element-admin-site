...
* [**medium-**_**editor**_](https://github.com/yabwe/medium-editor) 大名鼎鼎的 medium 的富文本\(非官方出品\)，但完成度还是不很不错，拓展性也不错。不过我觉得大部分用户还是会不习惯 medium 这种写作方式的。
...
> 由于目前使用 npm 安装 `Tinymce` 方法比较负责复杂而且还有一些问题\(日后可能会采用该模式\)且会大大增加编译的时间所以暂时不准备采用。:space\_invader:
